...
t_line *upper_hull( t_point *points, int num_point, t_point p1, t_point pn, t_line *lines, int *num_line, int *capacity){
    if(!num_point){
        lines = realloc(lines, sizeof(t_line) * (*num_line + 1));
    ...
    float dist_max = 0 ;
    for(int i = 0; i < num_point; ++i){
        float temp = distance(a, b, c, points[i]);
        if(dist_max < temp){
            dist_max = temp;
            max = points[i];
        }
    ...
    assert( s4 != NULL);
    int n1, n2;
    separate_points(points, num_point, p1, max, s1, s2, &n1, &n2);
    lines = upper_hull(s1, n1, p1, max, lines, num_line, capacity);
    separate_points(s2, n2, max, pn, s3, s4, &n1, &n2);
    lines = upper_hull(s3, n1, max, pn, lines, num_line, capacity);
    free(s1);
    free(s2);
    free(s3);
    free(s4);
